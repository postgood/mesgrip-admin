...
class LoginController {
	...
	signinInit = () => {
		...
		if(logBoxWidth > width){
			width = document.body.clientWidth - 20
			$('.logBox').width(width);
			$('.logBox').css('left', '50%').css('margin-left', ((width/2) * -1) +'px');
		}else{
		...
		_api.ajaxformdata(function(rdata){ 
					if(rdata.code == 0) {
						...
						//$(".logoImage").empty();
						$(".copy").empty();
						//$(".logoImage").append('<img src="'+ 'https://kprintfactory.s3.ap-northeast-2.amazonaws.com'+d.fileInfo[0].path.replace('kprintfactory/', '') +'" alt="">');
						$(".logoImage").attr('src', 'https://kprintfactory.s3.ap-northeast-2.amazonaws.com'+d.fileInfo[0].path.replace('kprintfactory/', ''));
